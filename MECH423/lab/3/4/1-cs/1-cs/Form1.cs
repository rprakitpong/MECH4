...
namespace _1_cs
{
    public partial class Form1 : Form
    {
        ...
        private void writeToFile(int pos, DateTime time)
        {
            if (saveToFileCheck.Enabled)
            {
                sb.AppendLine(time.ToString("HH:mm:ss.ffff") + "," + pos.ToString());
            }
        }

        private void button1_Click_1(object sender, EventArgs e)
        {
            saveToFileCheck.Enabled = false;
            using (StreamWriter sr = new StreamWriter(filenameBox.Text, true))
            {
                sr.WriteLine(sb.ToString());
            }
            saveToFileCheck.Enabled = true;
        }

        private void saveToFileCheck_CheckedChanged(object sender, EventArgs e)
        {
            if (saveToFileCheck.Enabled == true)
            {
